...
app.post('/api/training-clubs/upload', adminAuth, (req, res) => {
  const form = formidable({
    multiples: false,
    maxFileSize: 10 * 1024 * 1024, // 10MB limit
    allowEmptyFiles: false
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const file = getSingleValue(files, 'file');
    const clubId = getSingleValue(fields, 'clubId') || 'new';
    ...
    try {
      ...
      const randomSuffix = Math.random().toString(36).substring(2, 8);
      const safeFilename = (file.originalFilename || 'image').replace(/[^a-zA-Z0-9.-]/g, '_');
      const fileName = `training-clubs/${clubId}-${Date.now()}-${randomSuffix}-${safeFilename}`;
